...
import Portada from "/src/assets/portada.svg";
import Gunelve from "/src/assets/gunelve.svg";

const Hero = () => {
  const handleScrollToStory = () => {
    const storyElement = document.getElementById('relatos');
    if (storyElement) {
      const offsetTop = storyElement.offsetTop - 80;
      window.scrollTo({
        top: offsetTop,
        behavior: 'smooth'
      });
  ...
  return (
    <section id="inicio" className="min-h-screen relative overflow-hidden bg-gray-50">
      {/* Background illustration pattern */}
      <div className="absolute inset-0 opacity-15 h-full">
        <img src={Portada}
          className="h-full w-auto object-cover mx-auto"
          alt="Fondo portada"></img>
      </div>
      ...
      <div className="absolute inset-0">
        ...
        >
          <path 
            d="M0 500 Q300 400 600 420 Q900 440 1200 400 Q1300 390 1400 410 L1400 600 L0 600 Z" 
            fill="#1d4ed8" 
            opacity="0.8"
          />
        </svg> 
        ...
        >
          <path 
            d="M0 520 Q200 480 400 500 Q600 520 800 490 Q1000 460 1200 480 Q1300 485 1400 490 L1400 600 L0 600 Z" 
            fill="#2563eb"
          />
        </svg> 
      </div>
      ...
      {/* Content */}
      <div className="relative z-10 min-h-screen flex flex-col items-center justify-end pb-10 px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-4xl mx-auto">
          {/* Main title with equal width constraint */}
          <div className="mb-8">
            <h1 className="font-trajan text-6xl sm:text-7xl md:text-8xl lg:text-9xl text-gray-900 mb-4 tracking-widemx-auto md:max-w-3xl">
              TRALKÁN
            </h1>
            <h2 className="font-trajan text-2xl sm:text-3xl md:text-4xl lg:text-5xl text-gray-800 font-light tracking-widestmx-auto md:max-w-3xl">
              Susurros de la ÑukeMapu
            </h2>
          </div>

          {/* Authors */}
          <div className="mb-10">
                <div className="text-xl sm:text-2xl md:text-3xl text-white font-medium mx-auto md:max-w-3xl flex justify-center gap-8 sm:gap-12 md:gap-16">
              <span>Millapán</span>
              <span>Moraga</span>
              <span>Muñoz</span>
            </div>
          ...
          <Link 
            to="/conoce-a-tralkan"
            className="bg-blue-900 text-white px-8 py-4 rounded-lg text-lg font-semibold hover:bg-blue-800 transform hover:scale-105 transition-all duration-300 shadow-lg hover:shadow-xl flex items-center gap-2 mx-auto w-fit"
          > 
            <img src={Gunelve} className="w-7 h-7 fill-current"></img>
            Explora los Relatos
          </Link>
        </div>
      </div>
